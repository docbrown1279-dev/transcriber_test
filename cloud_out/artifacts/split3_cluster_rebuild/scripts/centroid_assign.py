...
class SpeakerGallery:
    ...
    def clone(self) -> SpeakerGallery:
        """Deep-copy speakers (float64 centroids) for a fresh hypothesis replay."""
        gallery = SpeakerGallery(threshold=self.threshold)
        for slot in self.speakers:
    ...
    def match_existing(
        ...
        """Nearest-centroid assign; unmatched rows stay ''. Does not mutate gallery."""
        n = len(embeddings)
        assigned = [""] * n
        log_windows: list[dict[str, Any]] = []
        if n == 0:
            return [], {"matched": 0, "unmatched": 0, "windows": []}
        if not self.speakers:
            for i in range(n):
                log_windows.append(
                    {
            ...
            return assigned, {"matched": 0, "unmatched": n, "windows": log_windows}
        ...
        for i in range(n):
            ...
            d_best = float(dist[i, int(order[0])])
            d_second = float(dist[i, int(order[1])]) if len(order) > 1 else None
            best_id = str(self.speakers[int(order[0])]["id"])
            hit = d_best <= self.threshold
            if hit:
                assigned[i] = best_id
                matched += 1
            log_windows.append(
                {
                    ...
                    "assigned": assigned[i],
                    "best_id": best_id,
                    "min_distance": round(d_best, 4),
                    "second_distance": None if d_second is None else round(d_second, 4),
                    "new_speaker": not hit,
                }
            )
        return assigned, {
    ...
    def leftover_ahc(self, embeddings: np.ndarray, assigned: list[str]) -> dict[str, Any]:
        """AHC unmatched windows into new gallery ids. Mutates ``assigned`` and gallery."""
        unmatched_idx = [i for i, spk in enumerate(assigned) if not spk]
        new_ids: list[str] = []
        leftover_label_to_id: dict[int, str] = {}
        if not unmatched_idx:
            return {"new": 0, "new_speaker_ids": [], "unmatched_idx": []}
    ...
    def blend_assignments(self, embeddings: np.ndarray, assigned: list[str]) -> None:
        """Count-weighted blend of prior centroid + this-part windows; L2-normalize."""
    ...
    def refresh_centroids_keep_counts(
        self, embeddings: np.ndarray, assigned: list[str]
    ) -> None:
        """Move centroids using labeled windows; leave n_windows at the prior count."""
        by_spk: dict[str, list[int]] = {}
        for i, spk in enumerate(assigned):
            if not spk:
                continue
