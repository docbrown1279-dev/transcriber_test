...
| Склейка фраз / absorb спикеров | `base.yaml` → `diarization.merge` (`vad_premerge_gap_sec`, `same_speaker_gap_sec`, `absorb_turn_shorter_than_sec`, `min_speaker_speech_sec` — EOS: короткие SPEAKER_* к ближайшему) |
...
| TTFT file-split (early TOC) | `pipeline.ttft_split` + `pipeline.ttft.*` (demo: `true`; part length band **min/max/target = 240/360/300** с = 4–6 мин) |
