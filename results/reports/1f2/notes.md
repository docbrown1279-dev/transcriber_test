...
Эталон меток — Stage 1e pyannote 3.1 (`results/reports/1f/baseline/pyannote31/`), **не** человеческое золото. `eval/` не читался. GigaAM / WER / заголовки / полная запись не запускались. Победитель продукта не объявляется.
